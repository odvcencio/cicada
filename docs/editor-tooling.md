...
`cicada view examples/first-acid.cicada -o first-acid.html` exports a read-only score view. It keeps syntax highlighted source beside pitch and drum grids, with the song and tracks below. The same command accepts validated semantic JSON and prints its canonical Cicada source. `cicada play` now supplies native playback and next-bar source reloads; a later studio will connect that transport to the editable visual views.
