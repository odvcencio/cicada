...
`cicada view examples/first-acid.cicada -o first-acid.html` exports a read-only score view. It keeps syntax highlighted source beside pitch and drum grids, with the song and tracks below. The same command accepts validated semantic JSON and prints its canonical Cicada source. `cicada studio examples/cicada-chorus.cicada` opens the editable source-linked view with native playback and next-bar source reloads. The chorus example uses the concise notation from the owner's language design.
