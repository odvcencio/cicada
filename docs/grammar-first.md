...
The current implementation compiles scores to note events and instrument graph IR, and renders custom mono graphs, acid, eleven drum lanes, authored kits, a drive insert, delay and reverb returns, and an optional music-bus compressor to WAV. Track compressor inserts, the worklet, and the workstation remain open.
