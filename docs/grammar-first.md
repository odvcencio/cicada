...
The current implementation compiles scores to note events and instrument graph IR, and renders custom mono graphs, acid, eleven drum lanes, authored kits, a drive insert, a delay return on send A, and a reverb return on send B to WAV. Compressor, the worklet, and the workstation remain open.
