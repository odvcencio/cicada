...
The current implementation compiles scores to note events and instrument graph IR, and renders custom mono graphs, acid, eleven drum lanes, authored kits, a drive insert, and a delay return on send A to WAV. Reverb, compressor, send B, the worklet, and the workstation remain open.
