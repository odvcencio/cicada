...
Built-in drum tracks keep the six legacy voices (`bd sd ch oh cp rs`). Each added lane (`lt mt ht cb cy`) uses one more voice when the track sets a parameter for that lane or a stored pattern has a hit on it. This includes patterns in scenes outside the song. Rest-only rows do not add voices. The source check, semantic check, native engine, and WASM image use this rule. The realtime cap remains 32 voices. In a built-in `engine.Config`, zero parameters leave an added lane off; set its default parameters to enable it before initialization. Live commands cannot enable a lane that was not allocated. Authored kits keep their explicit lane bindings.
