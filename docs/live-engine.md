...
Each Studio song block has a Play button. It starts song mode at that entry on the next audio read, including while playback is already running. The request names the entry by position and scene; the audio reader resolves its start bar against the active or newly offered score, so a saved duration edit cannot make the button seek to a stale bar. The reader discards buffered old audio, seeks the engine, reanchors the reported bar and step, and fades in the new output over five milliseconds. A missing entry reports an error and leaves the current score playing. The song then continues into later entries and loops according to the score.
