...
Each Studio song block has a Play button. It starts song mode at that entry on the next complete stereo-frame boundary in the audio reader, including while playback is already running. The request names the entry by position and scene; the audio reader resolves its start bar against the active or newly offered score, so a saved duration edit cannot make the button seek to a stale bar. The reader discards the remaining complete frames of buffered old audio, cancels queued scene and slot launches, seeks the engine, reanchors the reported bar and step, and fades in the new output over five milliseconds. A missing entry reports an error and leaves the current score playing. The song then continues into later entries and loops according to the score.

The song-start endpoint validates and compiles one source snapshot before offering it to the audio reader. Studio's file watcher also hashes and compiles the same byte snapshot; a failed compile leaves the last successful hash eligible for another attempt. An external save that changes the requested revision is rejected before the jump is queued.
