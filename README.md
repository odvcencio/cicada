...
The bounded live engine can audition acid and six-lane drums from 24-byte commands, play packed patterns with gate releases, and switch among sixteen slots on a beat or bar boundary. Edits made during playback take effect at the next bar. It renders through the dry mixer and limiter and emits 16-byte status messages. Build and exercise its TinyGo audio module with `make test-kernel-wasm`; the test compares native and WASM musical event logs. A host must call the module's `_initialize` export before `gosx_audio_track_kind` or `gosx_audio_init`. Configure track kinds before init; `gosx_audio_out_ptr` exposes planar float32 stereo with the right channel starting `maxBlock` frames after the left. Invalid wire input emits a fault and silences rendering. The current drum pattern command encodes one lane per step; a full multi-lane drum grid, scenes, song, custom graph loading, and DAW integration remain open.
