...
Run `cicada fmt` from a project directory to format every `.cicada` score under the nearest `cicada.mod`; with no manifest it uses the current directory. Nested projects are left to their own `fmt` run. `cicada fmt --check` reports every file that would change without writing. The command parses all selected scores before changing any file, so an invalid score stops the run. A filename still formats one score to stdout, and `-w` writes that file; JSON formatting remains available by filename. Source conversion expands phrases and normalizes pitch spelling; semantic comparison checks the resulting project rather than source text. An unedited source document can be printed byte for byte, including comments.
