...
// CompileMixerParams lowers the dry M0 track controls. `off` is the source
...
func CompileMixerParams(track notation.Track) (Mixer, error) {
	...
	for _, param := range track.Params {
		...
		case "pan":
			...
			mixer.Pan = value
		case "send_a", "send_b", "send_pre", "mute", "solo", "insert", "bus":
			return mixer, fmt.Errorf("mixer parameter %s is reserved for M1", param.Name)
		}
	}
	return mixer, validateDryMixer(mixer)
}

func validateDryMixer(mixer Mixer) error {
	...
	if mixer.SendA != 0 || mixer.SendB != 0 || mixer.SendPre || mixer.Solo || mixer.Insert != "none" || mixer.Bus != "music" {
		return fmt.Errorf("sends, solo, inserts, and non-music buses are reserved for M1")
	}
	return nil
}
