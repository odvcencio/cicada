...
// VerifyWAV checks the generated PCM container, musical duration, and signal
...
func VerifyWAV(path string, opts VerifyOptions) (VerifyReport, error) {
	...
	if opts.Bits != 24 || opts.Bars < 1 || opts.Bars > 256 {
		return report, fmt.Errorf("verify-wav requires 24 bits and 1 to 256 bars")
	}
	...
	if get32(4) != uint32(info.Size()-8) || get32(16) != 16 || get16(20) != 1 || get16(22) != 2 || get32(24) != uint32(opts.SampleRate) || get32(28) != uint32(opts.SampleRate*6) || get16(32) != 6 || get16(34) != 24 {
		return report, fmt.Errorf("WAV header does not match stereo PCM24 at %d Hz", opts.SampleRate)
	}
	dataBytes := int64(get32(40))
	if dataBytes%6 != 0 || info.Size() != 44+dataBytes+24 {
		return report, fmt.Errorf("WAV size or frame alignment is invalid")
	}
	report.Frames = dataBytes / 6
	...
	wantTail := uint32(math.Round(opts.TailSec * float64(opts.SampleRate)))
